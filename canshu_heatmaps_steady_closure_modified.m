...
% GSRT-nSIS：基于公式(33)+(50)闭合稳态的参数热图（>=版本，快速版）
%
% 【功能】
% 直接由 Section 3 的闭合稳态关系求 rho* 与 x*：
%   1) 对每一个参数点，不再做动力学长时间推进；
%   2) 直接联立公式(33) 与 公式(50)；
%   3) 输出对应的闭合稳态 rho* 与 x*；
%   4) 最后把三组参数扫描合并成一个 2×3 大图：
%        三列分别对应扫描三组参数；
%        第一行为 rho*，第二行为 x*。
%
% 【热图组别】
%   A. (kappa0, kappa1) -> (rho*, x*)，保留 kappa0 >= kappa1
%   B. (zeta0,  zeta1)  -> (rho*, x*)，保留 zeta0  >= zeta1
%   C. (beta0,  beta1)  -> (rho*, x*)，保留 beta1  >= beta0
%
% 【记号说明】
%   0 = N（non-adoption，不保护）
%   1 = A（adoption，保护）
%
%   kappa0 = kappa_N,   kappa1 = kappa_A
%   zeta0  = zeta_N,    zeta1  = zeta_A
%   beta0  = beta_N,    beta1  = beta_A
%
% 【重要说明】
%   1) 本程序不再跑动力学末态，而是直接解闭合稳态；
%   2) 因此这是一组“闭合稳态热图”，而不是“动力学末态热图”；
%   3) 不保存 csv / mat，仅保留工作区结果；
%   4) 实验结果图保存到“ 四月实验结果图 ”文件夹，保存 fig 与 jpg 两种格式。
%
% 【绘图改动】
%   1) ρ* 统一使用 turbo colormap；
%   2) x* 改为自定义蓝绿顺序色图（更冷静、与论文风格更一致）；
%   3) 仅在 ρ* in (β0,β1) 子图上叠加灰色虚线等值线，以增强高值区分辨率；
%   4) 其余计算、工作区输出、保存逻辑保持不变。
% ========================================================================
    clc; close all;
